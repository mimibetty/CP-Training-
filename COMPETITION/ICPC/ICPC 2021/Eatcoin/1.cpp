...
BigInt yyy(int x) {
    return x * x * (x+1) * (x+1) * (2*x*x + 2*x - 1) /12;
}

bool check(int mid) {
    for (int i = 1; i <= 1e4; i++) {
        BigInt sum = mid  - p*(i +  1) + yyy(i) * q;
        if (sum < 0) return 0; 
    }
    return 1;
}
...
int main() {
    ...
    // BigInt x,y;
    // long long lower = p, uper = 1e18;
    // while (lower < uper) {
    //     long long mid = (lower + uper)/2;
    //     if (check(mid)) uper = mid;
    //     else lower = mid + 1;
    //     cout << lower << ' ' << uper << ' ' <<mid << ' '<< check(mid) << endl;
    // }
