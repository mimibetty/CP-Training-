...
struct SegmentTree {
    ...
    int MaxLeast(int id, int l, int r, long long k) {
        ...
        if (l == r) return l;
        ll mid = (l + r) / 2;
        long long mx = query(1,1,n,l,mid).maxx;// st[id * 2].maxx; // so luong 1 trong (l,mid) 
        if (mx >= k) {
            // cout << "L  " <<"mx  " <<  mx << ' ' <<"mid  " <<  mid <<  endl;
            return MaxLeast(id * 2, l, mid, k);  
        } 
        else {
          // cout << "R  " <<"mx  " <<  mx << ' ' <<"mid  " <<  mid <<  endl;
...
int main() {
    ...
    for (int i = 1; i <= q; i++) {
        ...
        if (type == 1) {
            ...
            cin >> x >>l;
            l++;
            int ff = st.MaxLeast(1,l,n,x); 
            if (ff == n && x > st.query(1,1,n,l,n).maxx) cout << -1 << endl;
            else cout << ff - 1 << endl;            
        }
    }
