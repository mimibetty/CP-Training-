...
struct SegmentTree {
    ...
    int find(int id, int l, int r, int k) {
        ...
        if (l == r) return l;
        int mid = (l + r) / 2;
        long long num = st[id * 2].sum; // so luong 1 trong (l,mid) 
        if (num >= k) {
            // cout << "L  " <<"num  " <<  num << ' ' <<"mid  " <<  mid <<  endl;
            return find(id * 2, l, mid, k);  
        } 
        else {
          // cout << "R  " <<"num  " <<  num << ' ' <<"mid  " <<  mid <<  endl;
              return find(id * 2 + 1, mid + 1, r, k - num);
        }
    }
...
int main() {
    ...
    SegmentTree st(n);
    st.build(a, 1, 1, n);
    // cout <<"Cheell  " << st.query(1,1,n,1,2).sum << endl;
    // for (int i = 1; i <= 3; i++) {
    //     cout << "Began  " << i << endl;
    //     cout << st.find(1,1,n,i) << endl;
    // }
    // return 0;
    for (int i = 1; i <= q; i++) {
        int u, v;
