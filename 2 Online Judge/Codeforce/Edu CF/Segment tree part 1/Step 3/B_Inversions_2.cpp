...
struct SegmentTree {
    ...
    int find(int id, int l, int r,int x) {
        ...
        int mid = (l + r) / 2;
        int num = query(1,1,n,mid+1, n).cnt;
        if (num > x) {
            return find(id * 2 + 1, mid + 1, n, x);
        }
        else {
            return find(id * 2, l, mid, x);
        }
    }
};
// find(k,l,r)
// tìm vị trí x đầu tiên sao cho sum(x,n) = k
int main() {
    ...
    vector<int>ans;
    for (int i = n; i >= 1; i--) {
        int x = st.find(1,1,n, a[i]);
        ans.pb(x);
        // cout << x << endl;
        st.update(1,1,n,x);
        // for (int j = 1; j <= n; j++) {
        //     cout << st.query(1,1,n,j,j).cnt << ' ' ;
        // }
        // cout << endl;
    }
    reverse(All(ans));
