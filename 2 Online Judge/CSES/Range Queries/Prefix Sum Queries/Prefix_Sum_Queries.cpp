...
struct SegmentTree {
    ...
    void merge(Node& a, Node& b, Node& c) {
        a.maxSeg = max({b.maxSeg, b.sum + c.pref});
    ...
    void build(vector<int> &a, int id, int l, int r) {
        if (l == r) {
            st[id].sum = a[l];
            st[id].pref = st[id].maxSeg = max(0, a[l]);
            return;
        }
    ...
    void update(int id, int l, int r, int u, long long val) {
        if (l == r) {
            st[id].sum = val;
            st[id].pref = st[id].maxSeg = max(1ll* 0, val);
            return;
        }
...
int main() {
    ...
    for (int i = 1; i <= q; i++) {
        ...
        if (type == 1) {
            st.update(1,1,n,u,v);
            // for (int j = 1; j <= n; j++) {
            //     cout << st.query(1,1,n,j,n).maxSeg << ' ';
            // }
            // cout << endl;
        }
        else {
            cout << st.query(1, 1, n, u, v).maxSeg << endl;        
        }
