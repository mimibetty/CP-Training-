            ...
            for (int j2 = j1 + 1; j2 < limm; j2++) {
                for (int j3 = j2 + 1;  j3 < limm; j3++) {
                    ...
                    if (f3[mm] + ff[j1].fi + ff[j2].fi + ff[j3].fi == n) {
                        if (f2[z2] == n) {
                            res = max({res, f2[z2] * prize[2], prize[1] + prize[3]*(ff[j1].fi + ff[j2].fi +ff[j3].fi) });
                            continue;
                        }
                        if (ff[j1].fi == n) {
                            res = max({res, ff[j1].fi * prize[3], prize[1] + f2[z2]*prize[2] + prize[3]*(ff[j2].fi + ff[j3].fi) });
                            continue;
                        }
                        if (ff[j2].fi == n) {
                            res = max({res, ff[j2].fi * prize[3], prize[1] + f2[z2]*prize[2] + prize[3]*(ff[j1].fi + ff[j3].fi) });
                            continue;
                        }
                        if (ff[j3].fi == n) {
                            res = max({res, ff[j3].fi * prize[3], prize[1] + f2[z2]*prize[2] + prize[3]*(ff[j1].fi + ff[j2].fi) });
                            continue;
                        }


                        res = max({res,f2[z2] *prize[2] + prize[3]*(ff[j1].fi + ff[j2].fi + ff[j3].fi),
                        ...
                        res = max(res, prize[1] + prize[2] * f2[z2] + prize[3] * (ff[j2].fi + ff[j3].fi + ff[j1].fi));
                    }
                }
            }
        }       
